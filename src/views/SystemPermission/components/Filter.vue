  ...
  <div class="filter-wrap">
    <a-form layout="inline" @submit.prevent>
      <a-form-item label="名称">
        <a-input v-model:value="local.name" allowClear placeholder="权限名称" style="width:200px" />
      </a-form-item>
      ...
      <a-space>
        ...
        <a-button @click="ctx.onAdd()">新增权限</a-button>
      </a-space>
    </a-form>
  </div>
</template>
...
const local = reactive({ name: '', type: undefined, status: undefined })
function emitSearch(){ ctx.onSearch({ ...local }) }
function reset(){ local.name=''; local.type=undefined; local.status=undefined; emitSearch() }
</script>
