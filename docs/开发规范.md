...
## 模板素材 Web IDE 规范
- 范围：模板素材、邮件模板、楼层素材均采用 Web IDE 维护页面模板
- 列表页与详情页分离：
  - 列表页：在 `Material*/Index.vue` 渲染模板列表与分页；行“编辑”跳转到详情页
  - 详情页：在 `Material*Detail/Index.vue` 渲染编辑器与预览（读取 `query.id` 加载），支持保存
- 页面结构：
  - 列表：`components/List.vue`（编辑/删除，分页），`components/Toolbar.vue`
  - 详情：`components/Toolbar.vue`（名称 + 保存），`components/Editor.vue`，`components/Preview.vue`
- 数据接口：位于 `@apis/material/*Template.js`，列表与详情读取、保存写入（模拟）
- Mock 规范：
  - 列表：`mock/material/{template|email|floor}/list.json` → `{ items: [{id,name}] }`
  - 详情：`mock/material/{template|email|floor}/{id}.json` → `{ id, name, content }`
  - 保存：`mock/material/{template|email|floor}/saveOk.json` → `{ success: true }`
